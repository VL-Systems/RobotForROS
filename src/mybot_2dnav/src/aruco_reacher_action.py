...
class aruco_reacher(object):
    def __init__(self):
        ...
        rospy.loginfo("[aruco_reacher] waiting for move_base...")
    ...
    def fiducial_cb(self, msg):
        if len(msg.transforms) > 0:
            ...
            if self.locked_goal is None:
                ...
                rospy.loginfo("[aruco_reacher] goal sent.")                
                ...
                if dr > self.dr_threshold:
                    rospy.loginfo("[aruco_reacher] dr ({}) is higher then dr_threshold ({}), goal resent.".format(dr, self.dr_threshold))
                    self.sent_cmd_to_move_base(goal_transformed)
                    ...
                    if dyaw > self.dyaw_threshold:
                        rospy.loginfo("[aruco_reacher] dyaw ({}) is higher then dyaw_threshold ({}), goal resent.".format(dyaw, self.dyaw_threshold))
                        self.sent_cmd_to_move_base(goal_transformed)
    ...
    def move_action_done_cb(self, state, result):        
        rospy.loginfo("[aruco_reacher] move base action done with state [{}]".format(state))
        self.locked_goal = None
